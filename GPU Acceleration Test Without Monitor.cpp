...
void InitGraphics(void) {
	CUSTOMVERTEX vertices[] =
    {
		...
		{ 2000.0f, 50.0f, 0.5f, 1.0f, D3DCOLOR_XRGB(0, 0, 255), },
        { 3520.0f, 2000.0f, 0.5f, 1.0f, D3DCOLOR_XRGB(0, 255, 0), },
        { 120.0f, 1800.0f, 0.5f, 1.0f, D3DCOLOR_XRGB(255, 0, 0), },
    };
...
void InitD3D(HWND hwnd) {
	...
	//Log GPU Information
	fprintf(logfile, "Adapter Count: %d\n", d3d->GetAdapterCount());
	D3DDISPLAYMODE dmode;
	d3d->GetAdapterDisplayMode(D3DADAPTER_DEFAULT, &dmode);
	fprintf(logfile, "\nDisplay Mode: \n");
	fprintf(logfile, "Width: %d\n", dmode.Width);
	fprintf(logfile, "Height: %d\n", dmode.Height);
	fprintf(logfile, "RefreshRate: %dHz\n", dmode.RefreshRate);

	HMONITOR hmon;
	hmon = d3d->GetAdapterMonitor(D3DADAPTER_DEFAULT);
	if(hmon != 0) {
		//fprintf(logfile, "\nGetAdapterMonitor:  D3DADAPTER_DEFAULT  \n");
		//fprintf(logfile, "\nHMONITOR::unused: %d\n", hmon->unused);
	}
	...
	if (caps.DeviceType == D3DDEVTYPE_SW) fprintf(logfile, "D3DDEVTYPE_SW\n");
	else if(caps.DeviceType == D3DDEVTYPE_HAL) fprintf(logfile, "D3DDEVTYPE_HAL\n");
	else fprintf(logfile, "NOT SUPPORTED\n");



	fprintf(logfile, "\nCheckDeviceType:    \n");
	fprintf(logfile, "Adapter, DevType, AdapterFormat, BackBufferFormat, Windowed\n");
	HRESULT res = d3d->CheckDeviceType(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, D3DFMT_X8R8G8B8, D3DFMT_X8R8G8B8, TRUE);
	fprintf(logfile, "D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, D3DFMT_X8R8G8B8, D3DFMT_X8R8G8B8, TRUE = %ld\n", res);
	res = d3d->CheckDeviceType(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, D3DFMT_A8R8G8B8, D3DFMT_A8R8G8B8, TRUE);
	fprintf(logfile, "D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, D3DFMT_A8R8G8B8, D3DFMT_A8R8G8B8, TRUE = %ld\n", res);

	D3DADAPTER_IDENTIFIER9 adpid;
	d3d->GetAdapterIdentifier(D3DADAPTER_DEFAULT, NULL, &adpid);
	fprintf(logfile, "\nGetAdapterIdentifier: D3DADAPTER_DEFAULT\n");
	fprintf(logfile, "Description: %s\n", adpid.Description);
	fprintf(logfile, "Device: %s\n", adpid.DeviceName);
	fwprintf(logfile, L"DeviceId: %ld\n", adpid.DeviceId);
	fprintf(logfile, "Driver: %s\n", adpid.Driver);
	fprintf(logfile, "DeviceIdentifier: %ld\n", adpid.DeviceIdentifier);
	fprintf(logfile, "SubSysId: %d\n", adpid.SubSysId);
	...
	//d3ddp.PresentationInterval = D3DPRESENT_INTERVAL_IMMEDIATE;
	
	// Creates a graphics device interface
	res = d3d->CreateDevice( D3DADAPTER_DEFAULT,  // Select a graphics device 
					   D3DDEVTYPE_HAL,  // Hardware Abstraction Layer
					   0, //hwnd, 
				       D3DCREATE_SOFTWARE_VERTEXPROCESSING,  // do all 3d calc in software
					   &d3ddp, 
					   &d3ddev );
	fprintf(logfile, "CreateDevice: %x\n", res);
...
int i = 0;  float render_start, render_end;
int fps = 0; float fps_delta = 0;
void RenderFrame(void) {
	render_start = (float)timeGetTime();

	// clear the window to deep blue
	d3ddev->Clear(0, NULL, D3DCLEAR_TARGET, D3DCOLOR_XRGB(i*20, 40, 100), 0.1f, 0);  i++;
	...
	for(int j = 0; j < 100000; j++) {
		sprintf(buff, "%d", j);
		DrawMessage(font, (rand()%SCREEN_WIDTH), (rand()%SCREEN_HEIGHT), 255, (rand()%255), (rand()%255), (rand()%255), buff);
	}
	...
	render_end = (float)timeGetTime();
	fprintf(logfile, "\nFrame: %d   FrameTime: %f", i, fps_delta);
...
void CalcFPS(void) {
	...
	fps_delta = (currframetime - lastframetime) * 0.001f;
	fps = 1/fps_delta;
...
int frameNo = 0;
IDirect3DSurface9 *backbf;
HRESULT __stdcall hk_EndScene(IDirect3DDevice9 * pDevice)
{
	//take screen shot
	pDevice->GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO, &backbf);

	wchar_t buff[10];
	wsprintf(buff, L"%d", frameNo);
	wcscat(buff, L".bmp");
	D3DXSaveSurfaceToFile(buff, D3DXIFF_BMP, backbf, NULL, NULL);
	frameNo++;
